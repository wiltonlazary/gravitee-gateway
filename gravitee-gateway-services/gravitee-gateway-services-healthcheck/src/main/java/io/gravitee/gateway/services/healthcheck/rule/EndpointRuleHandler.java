...
public abstract class EndpointRuleHandler<T extends Endpoint> implements Handler<Long> {
    ...
    protected Future<HttpClientRequest> createHttpClientRequest(final HttpClient httpClient, URI request, io.gravitee.definition.model.services.healthcheck.Step step) {
    ...
    protected RequestOptions prepareHttpClientRequest(URI request, io.gravitee.definition.model.services.healthcheck.Step step) {
        ...
        if (step.getRequest().getHeaders() != null) {
            step.getRequest().getHeaders().forEach(
                    httpHeader -> options.putHeader(httpHeader.getName(), httpHeader.getValue()));
        }
    ...
    protected void runStep(T endpoint, io.gravitee.definition.model.services.healthcheck.Step step) {
            try {
                ...
                healthRequestPromise.onComplete(new Handler<AsyncResult<HttpClientRequest>>() {
                    @Override
                    public void handle(AsyncResult<HttpClientRequest> event) {
                        if (event.succeeded()) {
                            ...
                            healthRequest.response(new Handler<AsyncResult<HttpClientResponse>>() {
                                @Override
