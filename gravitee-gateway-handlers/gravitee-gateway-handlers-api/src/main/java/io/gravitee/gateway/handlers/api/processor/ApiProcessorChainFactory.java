...
public abstract class ApiProcessorChainFactory implements
    ...
    protected void add(StreamableProcessor<ExecutionContext, Buffer> processor) {
        add(new ProcessorSupplier<>(() -> processor));
    }

    protected void add(Processor<ExecutionContext> processor) {
        add(StreamableProcessor.toStreamable(processor));
    }
