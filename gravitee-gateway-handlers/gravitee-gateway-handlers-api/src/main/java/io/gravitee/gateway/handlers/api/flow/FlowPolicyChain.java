 ...
 */
public class FlowPolicyChain implements PolicyResolver {

    private final Flow flow;

    FlowPolicyChain(Flow flow) {
        this.flow = flow;
    }
...
/*
    ...
    public StreamableProcessor<ExecutionContext, Buffer> provide(ExecutionContext context) {
        ...
        // Convert steps to policies
        List<Policy> policies = steps.stream()
                .filter(Step::isEnabled)
                .map(new Function<Step, Policy>() {
                    @Override
                    public Policy apply(Step step) {
                        return policyManager.create(streamType, step.getPolicy(), step.getConfiguration());
                    }
