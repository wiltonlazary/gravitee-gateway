 ...
 */
public class HttpProxyConnection extends AbstractProxyConnection {
    ...
    protected final ProxyRequest proxyRequest;
    protected HttpProxyResponse proxyResponse;
    ...
    public ProxyConnection connect(HttpClient httpClient, int port, String host, String uri) {
        httpClientRequest = doRequest(httpClient, port, host, uri);
    ...
    protected HttpClientRequest doRequest(HttpClient httpClient, int port, String host, String uri) {
        ...
        httpClientRequest.handler(new io.vertx.core.Handler<HttpClientResponse>() {
            @Override
    ...
    protected HttpProxyResponse handleUpstreamResponse(final HttpClientResponse clientResponse) {
        this.proxyResponse = new HttpProxyResponse(clientResponse);
        ...
        clientResponse.endHandler(new io.vertx.core.Handler<Void>() {
            @Override
            public void handle(Void event) {
                // Write trailing headers to client response
                clientResponse.trailers().forEach(new Consumer<Map.Entry<String, String>>() {
                    @Override
                    public void accept(Map.Entry<String, String> header) {
                        proxyResponse.trailers().set(header.getKey(), header.getValue());
                    }
                });
